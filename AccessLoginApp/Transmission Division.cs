...
{
    public partial class Transmission_Division : Form
    {
        private OleDbConnection connection = new OleDbConnection();
        public Transmission_Division()
        {
            InitializeComponent();
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                command.CommandText = "insert into TransmissionDivision ([Letter Ref No],[Letter Date],[Letter Subject],[DGM (C&C)letter ref No],[C&C letter date],[Attachment letter Ref no],[Attachment letter date],[AGM (CS) letter ref no],[AGM (CS) letter date])values('" + txt_letterRef.Text + "','" + txt_letterdate.Value.ToString() + "','" + txt_letterSub.Text + "','" + txt_DGM.Text + "','" + date_cc.Value.ToString() + "','" + txt_attach.Text + "','" + date_attach.Value.ToString() + "','" + txt_AGMCSL.Text + "','" + date_AGMCSL.Value.ToString() + "')";
        ...
        private void btn_delete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                OleDbCommand command = new OleDbCommand();
                command.Connection = connection;
                String query = "delete from TransmissionDivision where [Letter Ref No]='" + txt_letterRef.Text + "'";
                MessageBox.Show(query);
                command.CommandText = query;
        ...
        private void Transmission_Division_Load(object sender, EventArgs e)
        {
            try
            {
                connection.Open();
                OleDbCommand command = new OleDbCommand();
                command.Connection = connection;
                String query = "select * from TransmissionDivision";
                command.CommandText = query;
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                OleDbCommand command = new OleDbCommand();
                command.Connection = connection;
                String query = "select * from TransmissionDivision where [Letter Ref No]='" + comboBox1.Text + "'";
                command.CommandText = query;
